...
class Paper:
    ...
    def __str__(self):
        ...
        days_str = (
            f"{days_since_date} days " if months_str == "" and years_str == "" else ""
        )
    ...
    def from_document(document: object):
        ...
        paper_date = datetime.strptime(
            document["header"]["datestamp"], Paper.date_format()
        )
        ...
        embedding = (
            document["embedding_gemini_embedding_001"]
            if "embedding_gemini_embedding_001" in document
            else None
        )
    ...
    def from_openreview_json(paper_json: dict, api_version: int):
        if api_version == 1:
            ...
            abstract = Paper.remove_null_bytes(
                paper_json["content"]["abstract"]["value"]
            )
        ...
        paper_date = datetime.strptime(
            paper_json["oversight_metadata"]["conference_date"], Paper.date_format()
        )
    ...
    def from_database_row(row: tuple):
        (
            ...
            document,
            update_date,
            embedding_gemini_embedding_001,
            source,
            abstract,
            title,
            link,
